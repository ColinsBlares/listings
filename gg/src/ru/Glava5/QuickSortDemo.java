...
class QuicSort{
    static void sort(char items[]){
        quicsort(items, 0, items.length -1);
    }

    private  static void quicsort (char items[], int left, int right){
        int top, bottom;
        ...
        if (left < bottom) quicsort(items, left, bottom);
        if (top < right) quicsort(items, top, right);
    }
}
